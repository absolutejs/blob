...
All notable changes to `@absolutejs/blob`.
...
- **Independently enforce ZIP expansion, entry count, completeness and nesting limits before accepting ClamAV clean verdicts; ClamAV can otherwise silently skip oversized ZIP entries.** (`createClamdBlobInspector`)

## 0.6.1 — 2026-09-24

### Fixed

- **Document shared platform hosting and verify cancellation cannot commit a clean result.** (`createBlobInspectionProcessor`)

## 0.6.0 — 2026-09-24

### Security

- **Fail closed on malformed, truncated, premature, or oversized ClamAV replies; bound frames and release streams and sockets on cancellation or timeout.** (`createClamdBlobInspector`, `parseClamdResponse`, `inspectStoredBlob`, `BlobInspectionInput`)

### Added

- **Provide a queue-compatible inspection processor with revision-aware host callbacks and retryable unavailable verdicts.** (`createBlobInspectionProcessor`, `BlobInspectionUnavailableError`)

## 0.5.3 — 2026-09-24

### Security

- **Explicit private UploadThing ACL, bounded cancellable uploads, and retryable deletion failures.** (`collectBody`, `UploadThingClient`, `UploadThingBlobStoreOptions`)
