...
const makeMockS3Client = (
  ...
  const store = new Map<string, { body: Uint8Array; meta?: S3HeadOutput }>();
  ...
  const client: S3ClientLike = {
    ...
    getObject: async (input) => {
      ...
      if (entry.meta?.Metadata !== undefined)
        out.Metadata = entry.meta.Metadata;
      return out;
    },
    ...
    presignGetObject: async (input, opts) => {
      calls.push({
        input: { ...input, expiresIn: opts.expiresIn },
        op: "presign-get",
      });
...
describe("s3BlobStore", () => {
  ...
  test("streams segmented uploads and reports their exact size", async () => {
    ...
    expect(new TextDecoder().decode(bytes!)).toBe("streamed without buffering");
  });
  ...
  test("head maps ContentType, ContentLength, ETag (unquoted), LastModified", async () => {
    ...
    expect(head?.lastModified).toBe(new Date("2026-01-01T00:00:00Z").getTime());
  });
  ...
  test("a function typed against BlobStore accepts both local + S3", async () => {
    ...
    try {
      ...
      const roundTrip = async (
        store: { put: BlobBody | undefined } & typeof local,
      ) => {
        void store;
      };
