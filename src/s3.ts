...
const isNotFoundError = (error: unknown): boolean => {
  ...
  if (errorObj.name === "NoSuchKey" || errorObj.name === "NotFound")
    return true;
  if (errorObj.Code === "NoSuchKey" || errorObj.Code === "NotFound")
    return true;
...
export const s3BlobStore = (options: S3BlobStoreOptions): BlobStore => {
  ...
  const put = async (
    ...
    if (body instanceof ReadableStream) {
      ...
      if (maxBytes !== undefined && size > maxBytes)
        throw new BlobError(`blob exceeds ${maxBytes} bytes`, "PROVIDER_ERROR");
      uploadBody = bytes;
    }
  ...
  const list = async (listOptions: ListOptions = {}): Promise<ListResult> => {
